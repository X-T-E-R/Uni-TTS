...
def get_deflaut_character_name(models_path:str=None):
    ...
    try:
        ...
        if default_character is None:
            characters_and_emotions:dict = update_character_info(models_path)["characters_and_emotions"]
...
class GSV_Instance:
    def __init__(self, models_path=None, default_character=None, **kwargs):
        ...
        self.model_cache_dir = "cache/model_cache" if self.save_model_cache else ""
        tts_config = TTS_Config("")
        tts_config.device , tts_config.is_half = get_device_info()
        self.tts_pipline = TTS(tts_config, model_cache_dir=self.model_cache_dir)
    ...
    def get_characters(self) -> dict:
        return update_character_info(self.models_path)['characters_and_emotions']
