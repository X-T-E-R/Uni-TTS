...
i18n = I18nAuto(locale_path=os.path.join(os.path.dirname(__file__), "i18n/locale"))
...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str]=None):
        ...
        self.configs_path:str = os.path.join(os.path.dirname(__file__), "tts_infer.yaml")
...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config], model_cache_dir:str =""):
        ...
        if model_cache_dir not in ["", None]:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if self.save_cache:
            short_md5 = self.get_short_md5(weights_path)
            model_cache_path = os.path.join(self.model_cache_dir, f"vits_{short_md5}.pkl")
            
            if os.path.exists(model_cache_path):
                try:
                    with open(model_cache_path, "rb") as f:
                        self.vits_model, hps = pickle.load(f)
                    print(f"Load VITS model from cache: {model_cache_path}")
                    model_loaded = True
                except:
                    pass
        ...
        if not model_loaded:
            ...
            if self.save_cache:
                with open(model_cache_path, "wb") as f:
                    pickle.dump([self.vits_model, hps], f)
                print(f"Save VITS model to cache: {model_cache_path}")

        
        
    def get_short_md5(self, file_path:str = None):
        ...
        md5.update(f"{file_name}{modified_time}{size}".encode())
        md5.update(f"{device}{is_half}".encode())
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        if self.save_cache:
            short_md5 = self.get_short_md5(weights_path)
            model_cache_path = os.path.join(self.model_cache_dir, f"t2s_{short_md5}.pkl")
            
            if os.path.exists(model_cache_path):
                try:
                    with open(model_cache_path, "rb") as f:
                        self.t2s_model, config = pickle.load(f)
                    print(f"Load T2S model from cache: {model_cache_path}")
                    model_loaded = True
                except:
                    pass
        ...
        if not model_loaded:
            ...
            if self.save_cache:
                with open(model_cache_path, "wb") as f:
                    pickle.dump([self.t2s_model, config], f)
                print(f"Save T2S model to cache: {model_cache_path}")
    ...
    def _set_ref_spec(self, ref_audio_path):
        audio, sr = librosa.load(ref_audio_path, sr=self.configs.sampling_rate)
